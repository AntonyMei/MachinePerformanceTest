...
class StorageConfig:
    def __init__(self):
        ...
        self.replay_buffer_capacity = 3072
        # max mcts child count, used for determine block size
        self.max_child_visits_count = 16
        # block size list (max_len = 400)
        self.replay_buffer_block_size_list = [4096, 4 * (1024 ** 2), 4096, self.max_child_visits_count * 4096, 4096]
        ...
        self.mcts_storage_count = 2
        # capacity of each mcts storage
        self.mcts_storage_capacity = 8
        # block size of each entry in mcts storage
        self.mcts_block_size_list = [512 * (1024 ** 2), 128 * (1024 ** 2), 512 * (1024 ** 2), 128 * (1024 ** 2),
                                     128 * (1024 ** 2), 512 * (1024 ** 2), 128 * (1024 ** 2), 512 * (1024 ** 2)]
        ...
        self.batch_storage_capacity_worker = 8
        # block size for each entry in batch storage
        self.batch_storage_block_size_list = [256 * (1024 ** 2), 1 * (1024 ** 2), 1 * (1024 ** 2)]
        ...
        self.signal_queue_capacity = 1024
