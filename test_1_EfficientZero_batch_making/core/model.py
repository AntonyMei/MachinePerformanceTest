...
class BaseNet(nn.Module):
    ...
    def initial_inference(self, obs, rank=None) -> NetworkOutput:
        num = obs.size(0)
        ...
        if not self.training:
            ...
            # zero initialization for reward (value prefix) hidden states
            if rank is None:
                reward_hidden = (torch.zeros(1, num, self.lstm_hidden_size, device='cuda'),
                                 torch.zeros(1, num, self.lstm_hidden_size, device='cuda'))
            else:
                reward_hidden = (torch.zeros(1, num, self.lstm_hidden_size, device=f'cuda:{rank}'),
                                 torch.zeros(1, num, self.lstm_hidden_size, device=f'cuda:{rank}'))
